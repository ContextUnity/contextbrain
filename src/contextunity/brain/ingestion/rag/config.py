...
- CU_BRAIN_CONFIG_PATH: Direct path to settings.toml config file
- CU_BRAIN_ASSETS_PATH: Path to assets folder (config derived as {assets}/settings.toml)

Priority: CU_BRAIN_CONFIG_PATH > CU_BRAIN_ASSETS_PATH > package default
"""
...
from contextunity.brain.core import get_env
...
ENV_CONFIG_PATH = "CU_BRAIN_CONFIG_PATH"
ENV_ASSETS_PATH = "CU_BRAIN_ASSETS_PATH"
...
def _resolve_config_path() -> Path:
    ...
    Priority:
    1. CU_BRAIN_CONFIG_PATH env var (direct path to config file)
    2. CU_BRAIN_ASSETS_PATH env var (derives config as {assets}/settings.toml)
...
def _log_config_not_found(config_path: Path, example_path: Path) -> None:
    ...
    msg_lines = [
        ...
        f"  cp {example_path} $CU_BRAIN_ASSETS_PATH/settings.toml",
        "",
...
def load_config(config_path: Path | None = None, *, force: bool = False) -> RagIngestionConfig:
    ...
    Environment Variables:
        CU_BRAIN_CONFIG_PATH: Direct path to config file (highest priority)
        CU_BRAIN_ASSETS_PATH: Path to assets folder (config = {assets}/settings.toml)
    """
