...
class Config(BaseModel):
    ...
    def load(cls, config_path: Path | str | None = None) -> "Config":
        ...
        # This is intentionally separate from ingestion's CU_BRAIN_CONFIG_PATH.
        core_config_path = get_env("CU_BRAIN_CORE_CONFIG_PATH")
    ...
    def _apply_env_overrides(self) -> None:
        ...
        # Model configuration
        if llm_val := get_env("CU_BRAIN_DEFAULT_LLM"):
            self.models.default_llm = llm_val
        if intent_val := get_env("CU_BRAIN_INTENT_LLM"):
            self.models.rag.intent.model = intent_val
        if suggestions_val := get_env("CU_BRAIN_SUGGESTIONS_LLM"):
            self.models.rag.suggestions.model = suggestions_val
        if generation_val := get_env("CU_BRAIN_GENERATION_LLM"):
            self.models.rag.generation.model = generation_val
        if no_results_val := get_env("CU_BRAIN_NO_RESULTS_LLM"):
        ...
        if location := (get_env("VERTEX_LOCATION") or get_env("CU_BRAIN_VERTEX_LOCATION")):
        ...
        if v := (
            ...
            or get_env("CU_BRAIN_VERTEX_DISCOVERY_ENGINE_LOCATION")
        ):
            self.vertex.discovery_engine_location = v
        if v := (
            get_env("VERTEX_DATA_STORE_LOCATION") or get_env("CU_BRAIN_VERTEX_DATA_STORE_LOCATION")
        ):
            self.vertex.data_store_location = v
        if credentials_path := (
            get_env("VERTEX_CREDENTIALS_PATH") or get_env("CU_BRAIN_VERTEX_CREDENTIALS_PATH")
        ):
        ...
        if v := get_env("CU_BRAIN_PROJECT_PATH"):
            self.project_path = v
