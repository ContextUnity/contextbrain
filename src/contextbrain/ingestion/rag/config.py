...
- CONTEXTBRAIN_CONFIG_PATH: Direct path to settings.toml config file
- CONTEXTBRAIN_ASSETS_PATH: Path to assets folder (config derived as {assets}/settings.toml)
...
ENV_ASSETS_PATH = "CONTEXTBRAIN_ASSETS_PATH"
...
def _resolve_config_path() -> Path:
    ...
    Priority:
    1. CONTEXTBRAIN_CONFIG_PATH env var (direct path to config file)
    2. CONTEXTBRAIN_ASSETS_PATH env var (derives config as {assets}/settings.toml)
...
def _log_config_not_found(config_path: Path, example_path: Path) -> None:
    ...
    msg_lines = [
        ...
        f"  cp {example_path} $CONTEXTBRAIN_ASSETS_PATH/settings.toml",
        "",
...
def load_config(config_path: Path | None = None, *, force: bool = False) -> RagIngestionConfig:
    ...
    Environment Variables:
        CONTEXTBRAIN_CONFIG_PATH: Direct path to config file (highest priority)
...
def get_assets_paths(config: RagIngestionConfig | None = None) -> dict[str, Path]:
    ...
    1. CONTEXTBRAIN_ASSETS_PATH env var
