...
Thanks for contributing to **ContextBrain**.
...
- All internal exceptions that cross module/transport boundaries should inherit from
  `contextbrain.core.exceptions.ContextbrainError`.
- Every `ContextbrainError` must have a stable, **non-empty** `code` string.
...
- `contextbrain.core` is the kernel: keep it knowledge-agnostic.
